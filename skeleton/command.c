...
static GSList* g_slist_tail_free_full(GSList* gs, GDestroyNotify free_func) {
    assert(gs != NULL && free_func != NULL);
    gpointer head = g_slist_nth_data(gs, 0u);
    GSList* result = g_slist_remove(gs, head); // head se vuelve una lista con un solo nodo
    free_func(head);
    return (result);
}
...
void scommand_pop_front(scommand self){
    ...
    self->comms_array = g_slist_tail_free_full(self->comms_array, free);
}
...
void pipeline_pop_front(pipeline self){
    ...
    self->scomms_array = g_slist_tail_free_full(self->scomms_array, scommand_destroy_void);
}
